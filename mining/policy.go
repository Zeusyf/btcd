...
type Policy struct {
	// BlockPrioritySize is the size in Tx for high-priority / low-fee
	// transactions to be used when generating a block template.
	BlockPrioritySize uint32

	// TxMinFreeFee is the minimum fee in Hao/1000 bytes that is
...
func CalcPriority(tx *wire.MsgTx, utxoView *viewpoint.UtxoViewpoint, nextBlockHeight int32) float64 {
