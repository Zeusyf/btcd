...
out:
	for {
		...
		case txVI := <-v.validateChan:
			...
			txIn := txVI.txIn
			utxo := v.view.Utxo.LookupEntry(txIn.PreviousOutPoint)
			if utxo == nil {
...
func newTxValidator(utxoView *viewpoint.ViewPointSet) *txValidator {
	//, flags ScriptFlags,	sigCache *SigCache, hashCache *HashCache) *txValidator {
	return &txValidator{
...
func checkBlockScripts(block *btcutil.Block, utxoView *viewpoint.UtxoViewpoint) error {
	// do nothing here. signature verification is done in SigVerify
	// ,	scriptFlags ScriptFlags, sigCache *SigCache,	hashCache *HashCache) error {
	...
	segwitActive := scriptFlags&ScriptVerifyWitness == ScriptVerifyWitness
...
func ValidateTransactionScripts(tx *btcutil.Tx, utxoView *viewpoint.UtxoViewpoint) error {
//	flags txscript.ScriptFlags, sigCache *txscript.SigCache,	hashCache *txscript.HashCache) error {
