...
const maxAlertSize = MaxMessagePayload - maxSignatureSize - common.MaxVarIntPayload - 1
...
const maxCountSetCancel = (maxAlertSize - fixedAlertSize - common.MaxVarIntPayload + 1) / 4
...
func (alert *Alert) Serialize(w io.Writer, pver uint32) error {
	err := common.WriteElements(w, alert.Version, alert.RelayUntil,
	...
	for i := 0; i < count; i++ {
		err = common.WriteElement(w, alert.SetCancel[i])
		if err != nil {
			return err
		}
	}

	err = common.WriteElements(w, alert.MinVer, alert.MaxVer)
	if err != nil {
		return err
	}
	...
	for i := 0; i < count; i++ {
		err = common.WriteVarString(w, pver, alert.SetSubVer[i])
		if err != nil {
			return err
		}
	}

	err = common.WriteElement(w, alert.Priority)
	if err != nil {
		return err
	}
	err = common.WriteVarString(w, pver, alert.Comment)
	if err != nil {
		return err
	}
	err = common.WriteVarString(w, pver, alert.StatusBar)
	if err != nil {
		return err
	}
	return common.WriteVarString(w, pver, alert.Reserved)
}
...
func (alert *Alert) Deserialize(r io.Reader, pver uint32) error {
	err := common.ReadElements(r, &alert.Version, &alert.RelayUntil,
		&alert.Expiration, &alert.ID, &alert.Cancel)
	if err != nil {
		return err
	}
	...
	for i := 0; i < int(count); i++ {
		err := common.ReadElement(r, &alert.SetCancel[i])
		if err != nil {
			return err
		}
	}

	err = common.ReadElements(r, &alert.MinVer, &alert.MaxVer)
	if err != nil {
		return err
	}
	...
	for i := 0; i < int(count); i++ {
		alert.SetSubVer[i], err = common.ReadVarString(r, pver)
		if err != nil {
			return err
		}
	}

	err = common.ReadElement(r, &alert.Priority)
	if err != nil {
		return err
	}
	alert.Comment, err = common.ReadVarString(r, pver)
	if err != nil {
		return err
	}
	alert.StatusBar, err = common.ReadVarString(r, pver)
	if err != nil {
		return err
	}
	alert.Reserved, err = common.ReadVarString(r, pver)
	return err
}
...
func (msg *MsgAlert) OmcDecode(r io.Reader, pver uint32, enc MessageEncoding) error {
	var err error

	msg.SerializedPayload, err = common.ReadVarBytes(r, pver, MaxMessagePayload,
	...
	msg.Signature, err = common.ReadVarBytes(r, pver, MaxMessagePayload,
		"alert signature")
	return err
}
...
// This is part of the Message interface implementation.
func (msg *MsgAlert) OmcEncode(w io.Writer, pver uint32, enc MessageEncoding) error {
	var err error
	...
	if slen == 0 {
		return messageError("MsgAlert.OmcEncode", "empty serialized payload")
	}
	err = common.WriteVarBytes(w, pver, serializedpayload)
	if err != nil {
		return err
	}
	return common.WriteVarBytes(w, pver, msg.Signature)
}
