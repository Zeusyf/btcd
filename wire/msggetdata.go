...
func (msg *MsgGetData) OmcDecode(r io.Reader, pver uint32, enc MessageEncoding) error {
	...
	if count > MaxInvPerMsg {
		...
		return messageError("MsgGetData.OmcDecode", str)
	}
...
// This is part of the Message interface implementation.
func (msg *MsgGetData) OmcEncode(w io.Writer, pver uint32, enc MessageEncoding) error {
	// Limit to max inventory vectors per message.
	count := len(msg.InvList)
	if count > MaxInvPerMsg {
		str := fmt.Sprintf("too many invvect in message [%v]", count)
		return messageError("MsgGetData.OmcEncode", str)
	}
...
func (msg *MsgGetData) MaxPayloadLength(pver uint32) uint32 {
	...
	return common.MaxVarIntPayload + (MaxInvPerMsg * maxInvVectPayload)
}
