...
const MaxMessagePayload = (1024 * 1024 * 64) // 64MB
...
const (
	...
	// BaseEncoding, SignatureEncoding will not encode separator and anything after it
	// FullEncoding will do.
	FullEncoding
)
...
type Message interface {
	OmcDecode(io.Reader, uint32, MessageEncoding) error
	OmcEncode(io.Writer, uint32, MessageEncoding) error
	Command() string
...
func makeEmptyMessage(command string) (Message, error) {
	...
	case CmdAckInvitation:
		msg = &MsgAckInvitation{}
...
func readMessageHeader(r io.Reader) (int, *messageHeader, error) {
	...
	hdr := messageHeader{}
	var command [common.CommandSize]byte
	common.ReadElements(hr, &hdr.magic, &command, &hdr.length, &hdr.checksum)
...
//func WriteMessageN(w io.Writer, msg Message, pver uint32, btcnet common.OmegaNet) (int, error) {
...
//func WriteMessage(w io.Writer, msg Message, pver uint32, btcnet common.OmegaNet) error {
...
func WriteMessageWithEncodingN(w io.Writer, msg Message, pver uint32,
	...
	var command [common.CommandSize]byte
	cmd := msg.Command()
	if len(cmd) > common.CommandSize {
		str := fmt.Sprintf("command [%s] is too long [max %v]",
			cmd, common.CommandSize)
	...
	var bw bytes.Buffer
	err := msg.OmcEncode(&bw, pver, encoding)
	if err != nil {
	...
	common.WriteElements(hw, hdr.magic, command, hdr.length, hdr.checksum)
...
func ReadMessageWithEncodingN(r io.Reader, pver uint32, btcnet common.OmegaNet,
	...
	// MsgVersion OmcDecode function requires it.
