...
func (msg *MsgCFHeaders) OmcDecode(r io.Reader, pver uint32, _ MessageEncoding) error {
	// Read filter type
	...
	// Read stop hash
	err = common.ReadElement(r, &msg.StopHash)
	if err != nil {
		return err
	}

	// Read prev filter header
	err = common.ReadElement(r, &msg.PrevFilterHeader)
	if err != nil {
		return err
	}
	...
	if count > MaxCFHeadersPerMsg {
		str := fmt.Sprintf("too many committed filter headers for "+
			...
			MaxBlockHeadersPerMsg)
		return messageError("MsgCFHeaders.OmcDecode", str)
	}
...
// This is part of the Message interface implementation.
func (msg *MsgCFHeaders) OmcEncode(w io.Writer, pver uint32, _ MessageEncoding) error {
	// Write filter type
	...
	// Write stop hash
	err = common.WriteElement(w, msg.StopHash)
	if err != nil {
		return err
	}

	// Write prev filter header
	err = common.WriteElement(w, msg.PrevFilterHeader)
	if err != nil {
		return err
	}
	...
	if count > MaxCFHeadersPerMsg {
		str := fmt.Sprintf("too many committed filter headers for "+
			"message [count %v, max %v]", count,
			MaxBlockHeadersPerMsg)
		return messageError("MsgCFHeaders.OmcEncode", str)
	}
...
// differs from OmcDecode in that OmcDecode decodes from the bitcoin wire
...
func (msg *MsgCFHeaders) MaxPayloadLength(pver uint32) uint32 {
	...
	return 1 + chainhash.HashSize + chainhash.HashSize + common.MaxVarIntPayload +
