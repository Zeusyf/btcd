...
const DefaultUserAgent = "/omgwire:0.5.0/"
...
func (msg *MsgVersion) OmcDecode(r io.Reader, pver uint32, enc MessageEncoding) error {
	buf, ok := r.(*bytes.Buffer)
	if !ok {
		return fmt.Errorf("MsgVersion.OmcDecode reader is not a " +
			"*bytes.Buffer")
	}

	err := readElements(buf, &msg.ProtocolVersion, &msg.Services,
		(*common.Int64Time)(&msg.Timestamp))
	if err != nil {
		return err
	}
...
// This is part of the Message interface implementation.
func (msg *MsgVersion) OmcEncode(w io.Writer, pver uint32, enc MessageEncoding) error {
	...
	err = common.WriteVarString(w, pver, msg.UserAgent)
	if err != nil {
		return err
	}
