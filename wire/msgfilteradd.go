...
func (msg *MsgFilterAdd) OmcDecode(r io.Reader, pver uint32, enc MessageEncoding) error {
	if pver < BIP0037Version {
		str := fmt.Sprintf("filteradd message invalid for protocol "+
			"version %d", pver)
		return messageError("MsgFilterAdd.OmcDecode", str)
	}

	var err error
	msg.Data, err = common.ReadVarBytes(r, pver, MaxFilterAddDataSize,
		"filteradd data")
	return err
}
...
// This is part of the Message interface implementation.
func (msg *MsgFilterAdd) OmcEncode(w io.Writer, pver uint32, enc MessageEncoding) error {
	if pver < BIP0037Version {
		str := fmt.Sprintf("filteradd message invalid for protocol "+
			"version %d", pver)
		return messageError("MsgFilterAdd.OmcEncode", str)
	}
...
func (msg *MsgFilterAdd) MaxPayloadLength(pver uint32) uint32 {
	return uint32(common.VarIntSerializeSize(MaxFilterAddDataSize)) +
