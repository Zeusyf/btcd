...
func (c *Client) SetTxFeeAsync(fee btcutil.Amount) FutureSetTxFeeResult {
	cmd := btcjson.NewSetTxFeeCmd(fee.ToOMC())
	return c.sendCmd(cmd)
}
...
func (c *Client) SendToAddressAsync(address btcutil.Address, amount btcutil.Amount) FutureSendToAddressResult {
	...
	cmd := btcjson.NewSendToAddressCmd(addr, amount.ToOMC(), nil, nil)
	return c.sendCmd(cmd)
}
...
func (c *Client) SendToAddressCommentAsync(address btcutil.Address,
	...
	addr := address.EncodeAddress()
	cmd := btcjson.NewSendToAddressCmd(addr, amount.ToOMC(), &comment,
		&commentTo)
	return c.sendCmd(cmd)
}
...
func (c *Client) SendFromAsync(fromAccount string, toAddress btcutil.Address, amount btcutil.Amount) FutureSendFromResult {
	...
	cmd := btcjson.NewSendFromCmd(fromAccount, addr, amount.ToOMC(), nil,
		nil, nil)
	return c.sendCmd(cmd)
}
...
func (c *Client) SendFromMinConfAsync(fromAccount string, toAddress btcutil.Address, amount btcutil.Amount, minConfirms int) FutureSendFromResult {
	addr := toAddress.EncodeAddress()
	cmd := btcjson.NewSendFromCmd(fromAccount, addr, amount.ToOMC(),
		&minConfirms, nil, nil)
	return c.sendCmd(cmd)
}
...
func (c *Client) SendManyAsync(fromAccount string, amounts map[btcutil.Address]btcutil.Amount) FutureSendManyResult {
	...
	for addr, amount := range amounts {
		convertedAmounts[addr.EncodeAddress()] = amount.ToOMC()
	}
...
func (c *Client) MoveAsync(fromAccount, toAccount string, amount btcutil.Amount) FutureMoveResult {
	cmd := btcjson.NewMoveCmd(fromAccount, toAccount, amount.ToOMC(), nil,
		nil)
	return c.sendCmd(cmd)
}
...
func (c *Client) MoveMinConfAsync(fromAccount, toAccount string,
	...
	cmd := btcjson.NewMoveCmd(fromAccount, toAccount, amount.ToOMC(),
		&minConfirms, nil)
	return c.sendCmd(cmd)
}
...
func (r FutureGetAssetResult) Receive() ([]Asset, error) {
	res, err := receiveFuture(r)
	if err != nil {
	...
	rassets := make([]Asset, len(assets))
	for i, r := range assets {
		rassets[i].OutPoint = r.OutPoint
		rassets[i].BlockMeta = r.BlockMeta
		rassets[i].PkScript = r.PkScript
		rassets[i].Received = r.Received
		rassets[i].FromCoinBase = r.FromCoinBase
		for _, m := range r.Amount.Value {
//			fmt.Println(k)
//			fmt.Println("type: ", reflect.TypeOf(m))
			if r.Amount.TokenType & 1 == 0 {
				rassets[i].Amount.Value = &token.NumToken{int64(m.(float64)), }
			} else {
				mb := m.([]interface {})
				h := chainhash.Hash{}
				for i := 0; i < 32; i++ {
//					fmt.Println("type: ", reflect.TypeOf(mb[i]))
					h[i] = byte(mb[i].(float64))
				}
				rassets[i].Amount.Value = &token.HashToken{Hash:h,}
			}
		}
...
func (c *Client) GetAssetAsync(account string) FutureGetAssetResult {
	cmd := btcjson.NewGetAssetCmd(&account)
	return c.sendCmd(cmd)
}

func (c *Client) GetAsset(account string) ([]Asset, error) {
...
func (c *Client) GetDBAsync(bucket string) FutureGetDBResult {
	cmd := btcjson.NewGetDBCmd(&bucket)
	return c.sendCmd(cmd)
}

func (c *Client) GetDB(bucket string) (map[string]string, error) {
	return c.GetDBAsync(bucket).Receive()
}
