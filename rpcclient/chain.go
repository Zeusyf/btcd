...
func (c *Client) GetTxOutAsync(txHash *chainhash.Hash, index uint32, mempool bool, locked bool) FutureGetTxOutResult {
	hash := ""
	if txHash != nil {
		hash = txHash.String()
	}

	cmd := btcjson.NewGetTxOutCmd(hash, index, &mempool, &locked)
	return c.sendCmd(cmd)
}

// GetTxOut returns the transaction output info if it's unspent and
// nil, otherwise.
func (c *Client) GetTxOut(txHash *chainhash.Hash, index uint32, mempool bool, locked bool) (*btcjson.GetTxOutResult, error) {
	return c.GetTxOutAsync(txHash, index, mempool, locked).Receive()
}

// FutureGetTxOutResult is a future promise to deliver the result of a
// GetTxOutAsync RPC invocation (or an applicable error).
type FutureGetDefineResult chan *Response

// Receive waits for the response promised by the future and returns a
// transaction given its hash.
func (r FutureGetDefineResult) Receive() (*btcjson.GetDefineResult, error) {
	res, err := receiveFuture(r)
	if err != nil {
		return nil, err
	}
	...
	// Unmarshal result as an gettxout result object.
	var defineInfo *btcjson.GetDefineResult
	err = json.Unmarshal(res, &defineInfo)
	if err != nil {
		return nil, err
	}
...
//
// See GetTxOut for the blocking version and more details.
func (c *Client) GetDefineAsync(kind uint32, txHash *chainhash.Hash, recursive bool) FutureGetDefineResult {
	hash := ""
	if txHash != nil {
		hash = txHash.String()
	}

	cmd := btcjson.NewGetDefineCmd(kind, hash, recursive)
	return c.sendCmd(cmd)
}

// GetTxOut returns the transaction output info if it's unspent and
// nil, otherwise.
func (c *Client) GetDefine(kind uint32, hash *chainhash.Hash, recursive bool) (*btcjson.GetDefineResult, error) {
	return c.GetDefineAsync(kind, hash, recursive).Receive()
}
